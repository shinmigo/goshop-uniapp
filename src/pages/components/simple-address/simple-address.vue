...
export default {
	...
	methods: {
		init() {
			...
			this.provinceDataList = this.usingProvinceData;
			this.cityDataList = this.usingCityData[this.pickerValueDefault[0]];
			this.areaDataList = this.usingAreaData[this.pickerValueDefault[0]][this.pickerValueDefault[1]];
			this.pickerValue = this.pickerValueDefault;
		},
		
		initDatas() {
			if (!this.usingCustomData) {//use default datas
				this.usingProvinceData = provinceData;
				this.usingCityData = cityData;
				this.usingAreaData = areaData;
			}
		},
		
		handPickValueDefault() {
			if (this.pickerValueDefault !== [0, 0, 0]) {
				if (this.pickerValueDefault[0] > this.usingProvinceData.length - 1) {
					this.pickerValueDefault[0] = this.usingProvinceData.length - 1;
				}
				if (this.pickerValueDefault[1] > this.usingCityData[this.pickerValueDefault[0]].length - 1) {
					this.pickerValueDefault[1] = this.usingCityData[this.pickerValueDefault[0]].length - 1;
				}
				if (this.pickerValueDefault[2] > this.usingAreaData[this.pickerValueDefault[0]][this.pickerValueDefault[1]].length - 1) {
					this.pickerValueDefault[2] = this.usingAreaData[this.pickerValueDefault[0]][this.pickerValueDefault[1]].length - 1;
				}
			}
		},
		pickerChange(e) {
			...
			if (this.pickerValue[0] !== changePickerValue[0]) {
				// 第一级发生滚动
				this.cityDataList = this.usingCityData[changePickerValue[0]];
				this.areaDataList = this.usingAreaData[changePickerValue[0]][0];
				changePickerValue[1] = 0;
				changePickerValue[2] = 0;
			} else if (this.pickerValue[1] !== changePickerValue[1]) {
				// 第二级滚动
				this.areaDataList = this.usingAreaData[changePickerValue[0]][changePickerValue[1]];
				changePickerValue[2] = 0;
			}
		...
		queryIndex(params = [], type = 'value') {
			...
			// 1.获取省份的index
			let provinceIndex = this.usingProvinceData.findIndex(res => res[type] == params[0]);
			let cityIndex = this.usingCityData[provinceIndex].findIndex(res => res[type] == params[1]);
			let areaIndex = this.usingAreaData[provinceIndex][cityIndex].findIndex(res => res[type] == params[2]);
			return {
				index: [provinceIndex, cityIndex, areaIndex],
				data: {
					province: this.usingProvinceData[provinceIndex],
					city: this.usingCityData[provinceIndex][cityIndex],
					area: this.usingAreaData[provinceIndex][cityIndex][areaIndex]
				}
			};
		},
		clear() {},
		...
		initWithData:function(data) {
			...
			var area = city.map(function(item){
				
				var tmp = item.map(function(value) {
					return value.children;
				});
				
				return tmp;
			});
			this.usingProvinceData = province;
			this.usingCityData = city;
			this.usingAreaData = area;
			this.init();
		}
	}
};
